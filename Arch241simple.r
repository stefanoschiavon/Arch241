intro {
  ...
  z<-x>2
  z
  v1=seq(1,10, by=0.5) #Variables are case sensitive: Stefano rememeber to mention the difference between = and <-
  V1='Ciao'
  v1;V1
  sd(v1)
  v2<-v1^2
...
load("~/Dropbox/2018 Arch 241/Arch241simple.rda");attach(Arch241simple);
save("Arch241simple", file="~/Dropbox/2018 Arch 241/Arch241simple.rda")
...
Arch241simpleMale1<-filter(Arch241simple, Gender=="Male");summary(Arch241simpleMale1) #usingtidyverse
...
mutate(Arch241simple,BMI = Weight/(Height^2)) #using tidyverse

by_gender<-group_by(Arch241simple, Gender)
summarise(by_gender, count = n(), age = mean(Age), weight=mean(Weight))
...
boxplot {
  ...
  Arch141Box+geom_boxplot(aes(fill=factor(AgeBin)))+ theme_bw()+xlab("Sex applied at birth")
}
...
example of machine learning {
  ...
  regressors<-c("Height","Gender","Age") 
...
Tydeverse {
  ...
  Arch241simpleMale1<-filter(Arch241simple, Gender=="Male")
